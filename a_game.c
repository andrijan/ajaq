...
void PrintMOTD(edict_t * ent)
{
	...
	if (!skipmotd->value) {
		 ...
		 */
		if (tgren->value > 0 || !(ir->value)) { //|| proxmine->value > 0 ) {
			char grenade_num[32];
			//char proxmine_num[32];
            ...
            /*
			if (proxmine->value > 0)
				sprintf(proxmine_num, "%d proximity mine%s", (int)proxmine->value, (int)proxmine->value == 1 ? "" : "s");
                */
...
void GetAmmo(edict_t * ent, char *buf)
{
	...
	if (ent->client->pers.weapon) {
		...
		case GRENADE_NUM:
			...
			return;
		case PROXMINE_NUM:
			ammo = INV_AMMO(ent, PROXMINE_NUM);
			sprintf(buf, "%d grenade%s", ammo, (ammo == 1) ? "" : "s");
			return;
		}
	}
