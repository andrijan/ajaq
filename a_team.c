...
void OpenWeaponMenu (edict_t * ent)
{
	...
	if ((int) wp_flags->value & ~(WPF_MK23 | WPF_GRENADE | WPF_PROXMINE))
	{
		pos = 4;
...
int UpdateJoinMenu (edict_t * ent)
{
    ...
    if (heroes->value)
        sprintf (team1players, ""); //ESJ Don't display # of players
    else
    ...
    if (heroes->value)
        return TEAM2; // ESJ Always join 2
    if (num1 > num2)
        return TEAM2;
    else if (num2 > num1)
        return TEAM1;
...
qboolean BothTeamsHavePlayers ()
{
    ...
    if (heroes->value) {
        if (players[1] == 0 && players[2] > 1) { // ESJ Make one the hero if necessary
            i = 0;
...
int WonGame (int winner)
{
	...
	else
	{
		...
		else
		{
            ...
            if (!heroes->value) // ESJ no team score
                teams[winner].score++;
