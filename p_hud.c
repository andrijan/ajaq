...
void DeathmatchScoreboardMessage (edict_t * ent, edict_t * killer)
{
	...
	//FIREBLADE
	if (!heroes->value && teamplay->value && !use_tourney->value) // ESJ
	{
		// DW: If the map ends
		if (level.intermissiontime) {
	...
	for (i = 0; i < total; i++)
	{
		...
		if (cl_ent == ent)
			tag = "tag1";
		else if (cl_ent == killer || (heroes->value && cl_ent->client->resp.team == 1)) // ESJ Hero is always the killer
			tag = "tag2";
		else
			tag = NULL;
		if (tag)
...
void Cmd_Score_f (edict_t * ent)
{
  ...
  {
    //FIREBLADE
    if (!heroes->value && teamplay->value && ent->client->scoreboardnum < 2)	// toggle scoreboards... ESJ
		{
...
void G_SetStats (edict_t * ent)
{
	...
	if (!ent->client->chase_mode)
	{
		...
		if (INV_AMMO(ent, PROXMINE_NUM))
		{
			ent->client->ps.stats[STAT_PROXMINE_ICON] = gi.imageindex ("a_m61frag");
			ent->client->ps.stats[STAT_PROXMINES] = INV_AMMO(ent, PROXMINE_NUM);
		}
		else
		{
			ent->client->ps.stats[STAT_PROXMINE_ICON] = 0;
		}
		...
		if (ent->client->pers.weapon)
		{
			...
			case PROXMINE_NUM:
				ent->client->ps.stats[STAT_AMMO_ICON] = gi.imageindex("a_m61frag");
				ent->client->ps.stats[STAT_AMMO] = INV_AMMO(ent, PROXMINE_NUM);
				break;
			case GRAPPLE_NUM:
