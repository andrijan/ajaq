...
void player_die(edict_t * self, edict_t * inflictor, edict_t * attacker, int damage, vec3_t point)
{
    ...
    else if ((((self->health < -35) && (meansOfDeath == MOD_HC)) ||
	...
	for (i = 0; i < game.num_items; i++) {
		...
		case PROXMINE_NUM:
			if ((int)wp_flags->value & WPF_PROXMINE) {
				ent->client->pers.inventory[i] = proxmine->value;
			}
			break;
		}
	}
...
void EquipClient(edict_t * ent) // ESJ if hero, switch to hero weapon, no pistol, no item
{
	gclient_t *client;
	gitem_t *item;
	edict_t etemp;
	...
	if (client->resp.item->typeNum == BAND_NUM) {
        if (!heroes->value || client->resp.team == 2) { // ESJ no grandes for hero
            band = 1;
            ...
            if (proxmine->value > 0)	// proximty mines is turned on
            {
                item = GET_ITEM(PROXMINE_NUM);
                client->pers.inventory[ITEM_INDEX(item)] = proxmine->value;
            }
        }

	}
    weapon = client->resp.weapon->typeNum;
    if (heroes->value) {
        if (client->resp.team == 1) {
            if (strcmp(heroweapon->string, "mp5") == 0)
                weapon = MP5_NUM;
            else if (strcmp(heroweapon->string, "m4") == 0)
                weapon = M4_NUM;
            else if (strcmp(heroweapon->string, "hc") == 0)
                weapon = HC_NUM;
            else if (strcmp(heroweapon->string, "m3") == 0)
                weapon = M3_NUM;
            else if (strcmp(heroweapon->string, "sniper") == 0)
                weapon = SNIPER_NUM;
            else if (strcmp(heroweapon->string, "dual") == 0)
                weapon = DUAL_NUM;
            else if (strcmp(heroweapon->string, "knife") == 0)
                weapon = KNIFE_NUM;
        }
    }
	...
	if ((int)wp_flags->value & WPF_MK23) {
		...
		if (band && (!heroes->value || client->resp.team == 2)) // ESJ bandolier doesn't help hero
