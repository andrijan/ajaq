...
qboolean Pickup_Weapon (edict_t * ent, edict_t * other)
{
    ...
    if (heroes->value && other->client->resp.team == 1) //ESJ Heroes don't pick up weapons.
        return false;
	...
	else if (ent->item->typeNum == PROXMINE_NUM)
	{
		if (teamplay->value && !teamdm->value && ctf->value != 2 && !band)
			return false;

		if (other->client->pers.inventory[index] >= other->client->proxmine_max)
			return false;
...
ShowGun (edict_t * ent)
{
  ...
  if (strcmp (pszIcon, "w_mk23") == 0)
    ...
    nIndex = 9;
  else if (strcmp (pszIcon, "a_prox") == 0)
    nIndex = 10;
  ...
  if (ent->client->weaponstate == WEAPON_RELOADING)
      ...
      else
	{
	  ...
	  switch (ent->client->curr_weap)
        ...
        if (!heroes->value || ent->client->resp.team == 2) // ESJ Heroes don't run out
...
#define PROXMINE_TIMER           3.0
#define PROXMINE_MINSPEED        400
#define PROXMINE_MAXSPEED        800

void
weapon_proxmine_fire (edict_t * ent, qboolean held)
{
  vec3_t offset;
  vec3_t forward, right;
  vec3_t start;
  ...
  timer = ent->client->grenade_time - level.time;
  speed =
    PROXMINE_MINSPEED + (PROXMINE_TIMER -
			timer) * ((PROXMINE_MAXSPEED -
				   PROXMINE_MINSPEED) / PROXMINE_TIMER);

  // Reset Grenade Damage to 1.52 when requested:
  if (use_classic->value)
    fire_proximity_mine (ent, start, forward, 170, speed, timer, 170 * 2, held);
  else
    fire_proximity_mine (ent, start, forward, GRENADE_DAMRAD, speed, timer,
		   GRENADE_DAMRAD * 2, held);
...
void proxmine_fire (edict_t * ent)
{
	vec3_t offset;
	vec3_t forward, right;
	vec3_t start;
	...
	fire_proximity_mine (ent, start, forward, damage, speed, timer, damage * 2, false);

	INV_AMMO(ent, PROXMINE_NUM)--;
	if (INV_AMMO(ent, PROXMINE_NUM) <= 0)
	{
		ent->client->newweapon = GET_ITEM(MK23_NUM);
		ChangeWeapon (ent);
		return;
...
void Weapon_Proxmine (edict_t * ent)
{
	...
	&& (ent->client->weaponstate != WEAPON_BUSY))
	{
		...
		&& ent->client->ps.gunframe <= GRENADE_THROW_LAST)))
		{
			...
			fire_proximity_mine(ent, ent->s.origin, vec3_origin, damage, 0, 2, damage * 2, false);
