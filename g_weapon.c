...
/*
   ...
   This if an internal support routine for the knife mod (weapons shooting knives) (added by NRGizeR)
   =================
   */

    void
fire_knife (edict_t * self, vec3_t start, vec3_t aimdir, int damage, int kick,
        int te_impact, int hspread, int vspread, int mod)
{
    ...
    //if (mod == MOD_SNIPER) gi.dprintf("SNIPER!!\n");
    //VectorNormalize(end);
    //gi.dprintf("Start: %2f, %2f, %2f\n", start[0], start[1], start[2]);
    //gi.dprintf("Aim: %2f, %2f, %2f\n", aimdir[0], aimdir[1], aimdir[2]);

    r = crandom () * hspread;
    u = crandom () * vspread;

    //gi.dprintf("u: %2f r: %2f\n", u, r);

    VectorMA (aimdir, 8192, aimdir, end);

    //gi.dprintf("End: %2f, %2f, %2f\n", end[0], end[1], end[2]);
    ...
    switch (mod)
    {
        case MOD_M3: speed = 1200; overshoot = 1750; break;
        case MOD_HC: speed = 1200; overshoot = 1700; break;
        case MOD_M4: speed = 1700; overshoot = 1300; break;
        case MOD_MP5: speed = 1400; overshoot = 1500; break;
        case MOD_SNIPER: speed = 2000; overshoot = 1000; break;
        case MOD_MK23:
        case MOD_DUAL: 
        default: speed = 1200; overshoot = 1400; break;
    }
    VectorMA (end, overshoot, up, end); //add a upwards angle, so that the shots will be fired longer when aiming "straight"	

    /* changed the damage from 200 to 70 to be able to shoot more knives into the target at hand before he/she dies :P
       the mp5 for example has 55 (for reference... */
    knife_throw(self, start, end, 70, speed); 
}
        ...
        int kick, int hspread, int vspread, int mod)
{
    ...
    if (knife_mod->value) {
        fire_knife (self, start, aimdir, damage, kick, TE_GUNSHOT, hspread, vspread, mod);
    } else {
        ...
        int kick, int hspread, int vspread, int mod)
{
    setFFState (self);
    if (knife_mod->value) {
        fire_knife (self, start, aimdir, damage, kick, TE_BULLET_SPARKS, hspread, vspread, mod);
    } else {
...
fire_bullet_sniper (edict_t * self, vec3_t start, vec3_t aimdir, int damage,
        int kick, int hspread, int vspread, int mod)
{
    //we get two bonus knives to make the sniper a bit better..
    setFFState (self);
    if (knife_mod->value) {
        fire_knife (self, start, aimdir, damage, kick, TE_GUNSHOT, hspread, vspread, mod);
        fire_knife (self, start, aimdir, damage, kick, TE_GUNSHOT, 50, 50, mod);
        fire_knife (self, start, aimdir, damage, kick, TE_GUNSHOT, 50, 50, mod);
        fire_knife (self, start, aimdir, damage, kick, TE_GUNSHOT, 50, 75, mod);
    } else {
        ...
        int kick, int hspread, int vspread, int count, int mod)
{
    ...
    //if this is the HC, reduce the number of knifes somewhat, it's a little too powerful right now :)
    if (mod == MOD_HC) count = (count * 3)/4; 
    if (knife_mod->value) {
        for (i = 0; i < count; i++)
            fire_knife (self, start, aimdir, damage, kick, TE_SHOTGUN, hspread, vspread, mod);
    } else {
        for (i = 0; i < count; i++)
...
Grenade_Explode (edict_t * ent)
{
    ...
    vec3_t origin;
    int mod;
    vec3_t dir, forward, right, up, end,start;
    float angle;
    static float x,y;
    float r,u;
    ...
    //throw 18 knifes all over the place :)
    if (knife_mod->value) {
        for (i = 0; i<18; i++)
        {
            VectorCopy(ent->s.origin, start);
            start[2] += 42;
            angle = i*20 * (M_PI * 2 / 360);
            x = sin (angle);
            y = cos (angle);

            dir[0] = x;
            dir[1] = y;
            dir[2] = 0.2; //slightly upwards
            ...
            VectorMA (dir, 8192, dir, end);
            VectorMA (end, r, right, end);
            VectorMA (end, u, up, end);

            knife_throw(ent->owner, start, end, 70, 1200);
        }
    }
...
/* Proximity Mines */
    void
fire_proximity_mine (edict_t * self, vec3_t start, vec3_t aimdir, int damage,
        int speed, float timer, float damage_radius, qboolean held)
{
    edict_t *grenade;
    vec3_t dir;
    ...
    VectorClear (grenade->mins);
    VectorClear (grenade->maxs);
    grenade->s.modelindex = gi.modelindex ("models/items/keys/red_key/tris.md2");
    grenade->owner = self;
    grenade->touch = Grenade_Touch;
...
void kick_attack (edict_t * ent)
{
    ...
    if (tr.ent->takedamage || KickDoor (&tr, ent, forward))
    {
        ...
        {
            if (!heroes->value || tr.ent->client->resp.team == 2) { // ESJ Heroes don't lose weapons
        ...
        csurface_t * surf)
{
    ...
    /* NRGizeR - added to fix the problem where the shotgun and or hc would shoot one knife
       and the rest would stick together (or collide with the first one */
    if (knife_mod->value)
        if (!strcmp(other->classname, "thrown_knife")) return;
    ...
    if (0 == Q_stricmp (other->classname, "func_explosive"))
    {
        ...
        if (other->takedamage)
        {
            //changed the knockback from 0 to 80 to create a little more action feeling to the knives for the knifemod :)
            T_Damage (other, ent, ent->owner, ent->velocity, ent->s.origin,
                    plane->normal, ent->dmg, 80, 0, MOD_KNIFE_THROWN);
        }
        else
        {
...
knife_throw (edict_t * self, vec3_t start, vec3_t dir, int damage, int speed)
{
    ...
    PRETRACE ();
    tr = gi.trace (start, NULL, NULL, knife->s.origin, knife, MASK_SHOT);
